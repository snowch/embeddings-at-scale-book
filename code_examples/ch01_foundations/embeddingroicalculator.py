...
def risk_adjusted_roi(
    potential_benefit, probability_of_success, implementation_cost, annual_operating_cost, years
):
    ...
    payback_period_years = (
        (implementation_cost / expected_annual_benefit) if expected_annual_benefit > 0 else 0
    )
...
class EmbeddingROICalculator:
    ...
    def calculate_total_roi(self, years=5, probability_of_success=0.8):
        ...
        total_annual_benefit = sum(
            b.get("total_annual_benefit", b.get("annual_value", 0)) for b in self.benefits.values()
        )
        ...
        roi = risk_adjusted_roi(
            ...
            annual_operating_cost=(self.costs["annual_operating"] + self.costs["annual_data"]),
            years=years,
        )
