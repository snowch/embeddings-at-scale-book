...
class EmbeddingBiasMonitor:
    ...
    def audit_for_bias(self, embeddings, metadata, protected_attributes):
        ...
        for attribute in protected_attributes:
            ...
            impact_ratio = self.measure_disparate_impact(embeddings, metadata, attribute)
                ...
                bias_report["bias_details"].append(
                    {
                        ...
                        "severity": "high"
                        if impact_ratio < 0.7 or impact_ratio > 1.43
                        else "medium",
                    }
                )

            # Test for embedding space separation
            separation = self.measure_embedding_separation(embeddings, metadata, attribute)
    ...
    def debias_embeddings(self, embeddings, metadata, protected_attribute):
        ...
        group_centroids = {
            group: embeddings[indices].mean(axis=0) for group, indices in groups.items()
        }
        ...
        debiased_embeddings = embeddings - np.outer(embeddings @ bias_direction, bias_direction)

        # Renormalize
        debiased_embeddings = debiased_embeddings / np.linalg.norm(
            debiased_embeddings, axis=1, keepdims=True
        )
