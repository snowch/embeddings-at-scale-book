...
class MultiTaskEmbeddingModel(nn.Module):
    ...
    def __init__(self, embedding_dim=512, num_categories=1000, num_brands=5000):
        ...
        self.shared_encoder = TransformerEncoder(dim=embedding_dim, depth=6, heads=8)
    ...
    def compute_loss(self, outputs, targets, task_weights):
        ...
        if "category" in targets:
            losses["category"] = F.cross_entropy(outputs["category_logits"], targets["category"])

        # Brand classification loss
        if "brand" in targets:
            losses["brand"] = F.cross_entropy(outputs["brand_logits"], targets["brand"])

        # Price regression loss
        if "price" in targets:
            losses["price"] = F.mse_loss(outputs["price_pred"].squeeze(), targets["price"])
...
for batch in train_loader:
    ...
    loss, task_losses = model.compute_loss(
        outputs, targets=batch["targets"], task_weights=task_weights
    )
