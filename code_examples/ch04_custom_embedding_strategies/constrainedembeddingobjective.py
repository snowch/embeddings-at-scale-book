...
class ConstrainedEmbeddingObjective:
    ...
    def search_with_constraints(self, query, k=20):
        ...
        final_results = self.constrained_reranking(candidates, constraints=self.constraints, k=k)
    ...
    def satisfies_constraints(self, selected, constraints):
        ...
        for constraint in constraints:
            if constraint["type"] == "diversity":
                ...
                # Check freshness
                recent_count = sum(1 for item in selected if item["days_since_published"] <= 30)
