...
The key innovation is the **attention mechanism**: when processing a word, the model can "look at" all other words in the sentence to understand meaning. Unlike Word2Vec (where "bank" always has the same embedding), transformers give "bank" different embeddings depending on whether the sentence mentions money or rivers.
...
The financial sentences (0.486) are more similar to each other than to river sentences, and vice versa—but the differences aren't dramatic because sentence embeddings capture the *entire* sentence meaning, not just the word "bank". The shared structure ("at the bank", "near the bank") creates some similarity across meanings.
