...
### Word2Vec: Static Word Embeddings {#sec-word2vec}
...
::: {.callout-tip}
## Key Resources
- [Word2Vec paper](https://arxiv.org/abs/1301.3781) (Mikolov et al., 2013) — introduced Word2Vec and skip-gram
- [Negative sampling paper](https://arxiv.org/abs/1310.4546) (Mikolov et al., 2013) — improved training efficiency
- [Gensim library](https://radimrehurek.com/gensim/) — popular Python library for training Word2Vec
:::

```{python}
#| echo: false
#| label: fig-skipgram-concept
#| fig-cap: "Skip-gram: given a target word, predict context words within a window. Here, 'sat' predicts 'the', 'cat', 'on', 'the'."
import matplotlib.pyplot as plt
...
for i, (word, x) in enumerate(zip(words, x_positions)):
    if i == target_idx:
        ...
        box = FancyBboxPatch((x-0.4, 0.9), 0.8, 0.6, boxstyle="round,pad=0.05",
                             facecolor='#FFEB3B', edgecolor='#F57C00', linewidth=2)
        ax.add_patch(box)
        ax.text(x, 1.2, word, ha='center', va='center', fontsize=12, fontweight='bold')
        ax.text(x, 0.5, 'target', ha='center', va='center', fontsize=9, color='#666')
    elif i < 5 and i != target_idx:  # Context words (window=2)
        box = FancyBboxPatch((x-0.4, 0.9), 0.8, 0.6, boxstyle="round,pad=0.05",
                             facecolor='#E3F2FD', edgecolor='#1976D2', linewidth=1.5)
        ax.add_patch(box)
        ax.text(x, 1.2, word, ha='center', va='center', fontsize=12)
        ax.text(x, 0.5, 'context', ha='center', va='center', fontsize=9, color='#666')
        # Arrow from target to context
        ax.annotate('', xy=(x, 1.5), xytext=(x_positions[target_idx], 1.5),
                   arrowprops=dict(arrowstyle='->', color='#1976D2', lw=1.5,
                                 connectionstyle='arc3,rad=0.2'))
    else:
        ax.text(x, 1.2, word, ha='center', va='center', fontsize=12, color='#999')

ax.text(5, 2.2, 'Window size = 2', ha='center', va='center', fontsize=10, style='italic', color='#666')
...
#| fig-cap: "Training pushes context words closer to target (green arrows) and negative samples farther away (red arrows)."
import matplotlib.pyplot as plt
import numpy as np
...
# Target word (center)
ax.scatter([2], [1.5], s=400, c='#FFEB3B', edgecolors='#F57C00', linewidth=2, zorder=5)
ax.text(2, 1.5, 'sat', ha='center', va='center', fontsize=11, fontweight='bold')
ax.text(2, 0.9, 'target', ha='center', va='center', fontsize=9, color='#666')

# Context words - being pulled closer (green arrows)
context_positions = [(0.8, 2.5), (3.2, 2.5)]
context_words = ['cat', 'on']
for (x, y), word in zip(context_positions, context_words):
    ax.scatter([x], [y], s=300, c='#E3F2FD', edgecolors='#1976D2', linewidth=1.5, zorder=5)
    ax.text(x, y, word, ha='center', va='center', fontsize=10)
    # Green arrow toward target
    dx, dy = (2 - x) * 0.3, (1.5 - y) * 0.3
    ax.annotate('', xy=(x + dx, y + dy), xytext=(x, y),
               arrowprops=dict(arrowstyle='->', color='#4CAF50', lw=2))

ax.text(2, 3.2, 'pull closer', ha='center', va='center', fontsize=9, color='#4CAF50', fontweight='bold')

# Negative words - being pushed away (red arrows)
neg_positions = [(0.5, 0.5), (3.5, 0.5)]
neg_words = ['mat', 'rug']
for (x, y), word in zip(neg_positions, neg_words):
    ax.scatter([x], [y], s=300, c='#FFEBEE', edgecolors='#E53935', linewidth=1.5, zorder=5)
    ax.text(x, y, word, ha='center', va='center', fontsize=10)
    # Red arrow away from target
    dx, dy = (x - 2) * 0.3, (y - 1.5) * 0.3
    ax.annotate('', xy=(x + dx, y + dy), xytext=(x, y),
               arrowprops=dict(arrowstyle='->', color='#E53935', lw=2))

ax.text(2, -0.2, 'push apart', ha='center', va='center', fontsize=9, color='#E53935', fontweight='bold')
...
embedding_dim = 4  # Small for demo; production uses 100-300 dimensions
word_to_idx = {w: i for i, w in enumerate(vocab)}

# Initialize two embedding matrices with small random values
np.random.seed(42)
...
# Each epoch = one pass through corpus. Multiple passes needed because
# each example only nudges embeddings slightly; repetition reinforces patterns.
...
### BERT: Contextual Token Embeddings {#sec-transformers-bert}
...
The key innovation is the **attention mechanism**: when processing a word, the model can "look at" all other words in the sentence to determine meaning. This is fundamentally different from Word2Vec:

- **Word2Vec**: Context matters during *training* (learning from nearby words), but produces *static* embeddings. Once trained, "bank" always maps to the same vector—the model can't tell which meaning you intend.
- **Transformers**: Context matters at *inference time*. Each time you encode a sentence, the model examines all surrounding words to compute a *contextual* embedding. "Bank" gets different vectors depending on whether the sentence mentions money or rivers.

::: {.callout-tip}
## Key Papers
- [Attention Is All You Need](https://arxiv.org/abs/1706.03762) (Vaswani et al., 2017) — introduced the transformer architecture
- [BERT: Pre-training of Deep Bidirectional Transformers](https://arxiv.org/abs/1810.04805) (Devlin et al., 2018) — applied transformers to create contextual embeddings
:::

```{python}
#| echo: false
#| label: fig-attention-concept
#| fig-cap: "Attention mechanism: when encoding 'bank', the model attends to other words to determine meaning. 'Money' and 'deposited' signal financial meaning."
import matplotlib.pyplot as plt
from matplotlib.patches import FancyBboxPatch
...
x_positions = [1, 2.3, 3.8, 5, 6.2, 7.8]
target_idx = 5  # "bank"

# Attention weights (stronger for relevant context words)
attention_weights = [0.1, 0.4, 0.5, 0.1, 0.1, 0.0]  # money and deposited get high attention

for i, (word, x) in enumerate(zip(words, x_positions)):
    if i == target_idx:
        # Target word being encoded
        box = FancyBboxPatch((x-0.4, 1.0), 0.9, 0.6, boxstyle="round,pad=0.05",
                             facecolor='#FFEB3B', edgecolor='#F57C00', linewidth=2)
        ax.add_patch(box)
        ax.text(x, 1.3, word, ha='center', va='center', fontsize=11, fontweight='bold')
        ax.text(x, 0.5, 'encoding', ha='center', va='center', fontsize=9, color='#666')
    else:
        # Context words with attention
        alpha = 0.3 + attention_weights[i] * 0.7
        box = FancyBboxPatch((x-0.4, 1.0), 0.9, 0.6, boxstyle="round,pad=0.05",
                             facecolor='#E3F2FD', edgecolor='#1976D2', linewidth=1.5, alpha=alpha)
        ax.add_patch(box)
        ax.text(x, 1.3, word, ha='center', va='center', fontsize=11, alpha=alpha)

        # Attention arrows (thickness = attention weight)
        if attention_weights[i] > 0.2:
            ax.annotate('', xy=(x, 1.6), xytext=(x_positions[target_idx], 1.6),
                       arrowprops=dict(arrowstyle='<-', color='#1976D2',
                                     lw=attention_weights[i]*4, alpha=0.7,
                                     connectionstyle='arc3,rad=0.3'))

ax.text(5, 2.6, '"money" and "deposited" signal financial meaning → bank = institution',
...
Unlike Word2Vec (where we showed training from scratch), we won't implement transformer training here. Why? Scale. Word2Vec trains in minutes on a laptop with a few sentences. BERT was trained on 3.3 billion words using 64 TPUs for 4 days. The architecture is also far more complex—multi-head attention, layer normalization, positional encodings, and masked language modeling objectives.

Instead, we use pre-trained models. The code below loads a model that's already learned contextual representations from massive text corpora:
...
sentences = [
    "I deposited money at the bank",      # financial-1
    "The bank approved my loan",          # financial-2
    "We had a picnic on the river bank",  # river-1
    "Fish swim near the bank",            # river-2
]

# Each sentence gets a single embedding capturing its full meaning
embeddings = model.encode(sentences)

print("Sentence similarity (same meaning = higher score):\n")
...
The financial sentences (0.486) are more similar to each other than to river sentences, and vice versa—but the differences aren't dramatic. Is this a limitation? It depends on your use case:

- **For sentence-level tasks** (semantic search, document clustering), this is fine. The embeddings correctly distinguish the sentence meanings, just not by a huge margin.
- **For word-sense disambiguation** (detecting that "bank" means different things), you need token-level embeddings instead of sentence embeddings. BERT produces distinct embeddings for "bank" in each context—the pooling step that creates a single sentence embedding averages this information away.

To get the contextual embedding for a specific word, use the underlying BERT model directly and extract the token's hidden state, rather than using a Sentence Transformer that pools all tokens together.

**Why transformers dominate:**

1. **Parallelization**: All words processed simultaneously. Earlier models (RNNs—Recurrent Neural Networks) processed words one at a time, left to right. This was slow and made training on large datasets impractical.
2. **Long-range dependencies**: Attention connects distant words directly. In "The cat that I saw yesterday at the park was sleeping", attention links "cat" to "sleeping" without processing all words in between.
3. **Transfer learning**: Pre-trained models work across many tasks without retraining from scratch.
4. **Scalability**: Performance improves predictably with more data and compute.

### Sentence Transformers: Whole-Sentence Embeddings {#sec-sentence-transformers}

The previous section covered BERT—a powerful architecture that produces *token-level* embeddings. But throughout this chapter, we've been using `SentenceTransformer` to get *sentence-level* embeddings. What's the connection?

Sentence Transformers [@reimers2019sentence] are BERT models fine-tuned specifically for producing useful sentence embeddings. The problem: raw BERT produces one embedding per *token*, not per sentence. The naive fix—averaging all token embeddings—works poorly because it weights filler words ("the", "a") equally with meaningful ones.

Sentence Transformers solve this using a *siamese* architecture: two identical BERT models process two sentences, then a pooling layer (typically mean or [CLS] token) produces one embedding each. Training uses **contrastive learning**—the model learns to produce similar embeddings for related sentences (paraphrases, question-answer pairs) and dissimilar embeddings for unrelated ones.

::: {.callout-tip}
## Key Resources
- [Sentence-BERT paper](https://arxiv.org/abs/1908.10084) (Reimers & Gurevych, 2019) — the original Sentence Transformers paper
- [Sentence Transformers library](https://www.sbert.net/) — documentation and pre-trained models
- [Hugging Face model hub](https://huggingface.co/sentence-transformers) — browse available models
:::

```{python}
#| echo: false
#| label: fig-sentence-transformer-training
#| fig-cap: "Sentence Transformer training: a siamese network processes sentence pairs through identical BERT encoders. Paraphrases are pushed together; unrelated sentences are pushed apart."
import matplotlib.pyplot as plt
from matplotlib.patches import FancyBboxPatch
...
# Left BERT encoder
box1 = FancyBboxPatch((0.5, 2), 2.5, 1.5, boxstyle="round,pad=0.1",
                       facecolor='#E3F2FD', edgecolor='#1976D2', linewidth=2)
ax.add_patch(box1)
ax.text(1.75, 2.75, 'BERT\nEncoder', ha='center', va='center', fontsize=10, fontweight='bold')

# Right BERT encoder (identical)
box2 = FancyBboxPatch((7, 2), 2.5, 1.5, boxstyle="round,pad=0.1",
                       facecolor='#E3F2FD', edgecolor='#1976D2', linewidth=2)
ax.add_patch(box2)
ax.text(8.25, 2.75, 'BERT\nEncoder', ha='center', va='center', fontsize=10, fontweight='bold')
ax.text(8.25, 2.1, '(shared weights)', ha='center', va='center', fontsize=8, style='italic', color='#666')

# Input sentences
ax.text(1.75, 1.5, '"The cat is sleeping"', ha='center', va='center', fontsize=9, style='italic')
ax.text(8.25, 1.5, '"A feline is napping"', ha='center', va='center', fontsize=9, style='italic')
ax.annotate('', xy=(1.75, 2), xytext=(1.75, 1.7),
           arrowprops=dict(arrowstyle='->', color='#666', lw=1.5))
ax.annotate('', xy=(8.25, 2), xytext=(8.25, 1.7),
           arrowprops=dict(arrowstyle='->', color='#666', lw=1.5))

# Output embeddings
ax.scatter([1.75], [4], s=200, c='#4CAF50', edgecolors='black', linewidth=1.5, zorder=5)
ax.scatter([8.25], [4], s=200, c='#4CAF50', edgecolors='black', linewidth=1.5, zorder=5)
ax.text(1.75, 4, 'e₁', ha='center', va='center', fontsize=10, fontweight='bold')
ax.text(8.25, 4, 'e₂', ha='center', va='center', fontsize=10, fontweight='bold')
ax.annotate('', xy=(1.75, 3.8), xytext=(1.75, 3.5),
           arrowprops=dict(arrowstyle='->', color='#666', lw=1.5))
ax.annotate('', xy=(8.25, 3.8), xytext=(8.25, 3.5),
           arrowprops=dict(arrowstyle='->', color='#666', lw=1.5))

# Similarity computation in middle
ax.annotate('', xy=(3.5, 4), xytext=(6.5, 4),
           arrowprops=dict(arrowstyle='<->', color='#4CAF50', lw=2))
ax.text(5, 4.3, 'cosine similarity', ha='center', va='center', fontsize=9, color='#4CAF50')

# Training signal
ax.text(5, 0.8, 'Paraphrase pair → push embeddings closer', ha='center', va='center',
        fontsize=10, color='#4CAF50', fontweight='bold')
ax.text(5, 0.4, 'Unrelated pair → push embeddings apart', ha='center', va='center',
...
The key insight: by training on millions of sentence pairs (from datasets like NLI, paraphrase corpora, and question-answer pairs), the model learns to capture semantic similarity—not just surface-level word overlap. This is why Sentence Transformers produce excellent embeddings out of the box, unlike raw BERT which needs task-specific fine-tuning.

For a deeper dive into contrastive learning techniques, see @sec-contrastive-learning. For training your own sentence embeddings, see @sec-custom-embedding-strategies.
