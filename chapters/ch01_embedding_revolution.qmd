...
\|\text{king}\| &= \sqrt{0.9^2 + 0.8^2 + 0.1^2} = \sqrt{0.86} \approx 0.93 \\[0.5em]
\|\text{queen}\| &= \sqrt{0.9^2 + 0.8^2 + 0.9^2} = \sqrt{1.66} \approx 1.29 \\[1em]
\text{cosine\_similarity} &= \frac{1.54}{0.93 \times 1.29} \approx 0.928 \rightarrow \text{Very similar!}
\end{aligned}
$$
...
Word embeddings map words to vectors, capturing semantic and syntactic relationships. Here we use a pre-trained sentence transformer model—we cover how these models are trained in @sec-contrastive-learning and @sec-self-supervised-learning:
...
Images can be embedded into vectors that capture visual content. We use a pre-trained ResNet model here—we explain how CNN-based image embedding models work in @sec-image-preparation:
