...
For deeper coverage: @sec-foundational-embedding-types covers the different data types you can embed (text, images, audio, etc.), @sec-advanced-embedding-patterns covers advanced types like hybrid embeddings and multi-vector representations, and @sec-embedding-model-fundamentals explains the underlying neural network architectures.
...
We explore each foundational type in depth in @sec-foundational-embedding-types. Production systems often combine and extend these foundations using advanced patterns—hybrid vectors, multi-vector representations, learned sparse embeddings, and more—which we cover in @sec-advanced-embedding-patterns. How embedding models learn these representations is explained in @sec-embedding-model-fundamentals.

### Embeddings in Action: Concrete Examples

**Word Embeddings**

In @lst-word-embedding-basics we used hand-crafted values to illustrate the concept. Here we use a pre-trained sentence transformer model ([all-MiniLM-L6-v2](https://huggingface.co/sentence-transformers/all-MiniLM-L6-v2)) to create real embeddings—we explain how these models learn in @sec-embedding-model-fundamentals:
