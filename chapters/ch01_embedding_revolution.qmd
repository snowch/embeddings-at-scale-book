...
# King vector (along x-axis-ish)
king1 = np.array([1.0, 0.15])
...
# Queen vector (small angle from king)
queen = np.array([0.9, 0.45])
...
ax1.text(0.35, 0.18, 'θ', fontsize=14, color='green', fontweight='bold')

ax1.set_title('Similar Vectors\ncos(θ) ≈ 0.94 → High similarity', fontsize=11, fontweight='bold')
ax1.axis('off')
...
ax2.set_title('Dissimilar Vectors\ncos(θ) ≈ 0.37 → Low similarity', fontsize=11, fontweight='bold')
ax2.axis('off')
...
%%| fig-cap: "Embedding workflow: encoding data once, then using fast vector search for decisions"
%%| fig-width: 100%
flowchart TB
    subgraph classification["Classification Workflow"]
        direction LR
        Q1[Query] --> NN1[Neural Network<br/>expensive, every time] --> D1[Decision]
    end

    subgraph embedding["Embedding Workflow"]
        subgraph index["Index Time (once per item)"]
            direction LR
            Items[Raw Data<br/>text, images, etc.] --> Encoder[Embedding Model<br/>BERT, CLIP, etc.] --> Vectors[Vectors<br/>768-1536 dims]
            Vectors --> VDB[(Vector Database)]
        end

        subgraph query["Query Time (per request)"]
            direction LR
            UserQuery[User Query] --> QEncoder[Same Encoder] --> QVec[Query Vector]
            QVec --> Search[ANN Search<br/>sub-millisecond]
            VDB2[(Vector Database)] --> Search
            Search --> Results[Top-K Results]
        end
    end

    style classification fill:#ffcccc,stroke:#cc0000
    style index fill:#ccffcc,stroke:#00cc00
    style query fill:#cce5ff,stroke:#0066cc
```
