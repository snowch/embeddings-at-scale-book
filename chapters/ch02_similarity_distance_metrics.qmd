...
# Left panel: vectors in same direction, offset perpendicular so dots don't overlap
origin = np.array([0, 0])
# Base direction and perpendicular offset
direction = np.array([3, 2])
perp = np.array([-2, 3]) / np.linalg.norm([-2, 3]) * 0.12  # perpendicular unit vector scaled

A = direction
B = direction * 1.5 + perp      # offset up
C = direction * 0.5 - perp      # offset down

# All dotted lines, offset so they don't overlap
ax1.annotate('', xy=A, xytext=origin, arrowprops=dict(arrowstyle='->', color='#2563eb', lw=2.5, linestyle=':'))
ax1.annotate('', xy=B, xytext=origin + perp, arrowprops=dict(arrowstyle='->', color='#16a34a', lw=2.5, linestyle=':'))
ax1.annotate('', xy=C, xytext=origin - perp, arrowprops=dict(arrowstyle='->', color='#dc2626', lw=2.5, linestyle=':'))
