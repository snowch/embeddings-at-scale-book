...
ax.text(0.3, 3.5, f'cos(A,C) = {sim_AC:.2f}', fontsize=11, color='#dc2626', fontweight='bold')
...
mid_AB = (A + B) / 2
mid_AC = (A + C) / 2
ax.text(mid_AB[0]+0.2, mid_AB[1]-0.4, f'd = {dist_AB:.2f}', fontsize=11, color='#16a34a', fontweight='bold')
ax.text(mid_AC[0]-0.8, mid_AC[1]-0.2, f'd = {dist_AC:.2f}', fontsize=11, color='#dc2626', fontweight='bold')
...
#| fig-cap: "Dot product combines angle AND magnitude. Vectors B and C point in the same direction as A, but B (longer) has a higher dot product. Compare to cosine similarity where B and C would be equal."
#| code-fold: true
#| code-summary: "Show visualization code"
...
origin = [0, 0]
A = np.array([2, 1.5])
B = np.array([4, 3])      # Same direction as A, 2x magnitude
C = np.array([1, 0.75])   # Same direction as A, 0.5x magnitude

# Draw vectors with different styles to distinguish overlapping lines
# Draw longest first (B), then medium (A), then shortest (C)
ax.annotate('', xy=B, xytext=origin, arrowprops=dict(arrowstyle='->', color='#16a34a', lw=4))
ax.annotate('', xy=A, xytext=origin, arrowprops=dict(arrowstyle='->', color='#2563eb', lw=3))
ax.annotate('', xy=C, xytext=origin, arrowprops=dict(arrowstyle='->', color='#dc2626', lw=2))

# Labels - position them away from the line
ax.text(A[0]-0.3, A[1]+0.35, 'A', fontsize=14, fontweight='bold', color='#2563eb')
ax.text(B[0]+0.15, B[1]+0.25, 'B (2×)', fontsize=12, fontweight='bold', color='#16a34a')
ax.text(C[0]+0.15, C[1]-0.35, 'C (0.5×)', fontsize=12, fontweight='bold', color='#dc2626')
...
# Add comparison box - position in lower right to avoid overlap
textstr = f'A · B = {dot_AB:.1f}  (longer → bigger)\nA · C = {dot_AC:.1f}  (shorter → smaller)\n\ncos(A,B) = cos(A,C) = {cos_AB:.2f}\n(same direction → same cosine)'
