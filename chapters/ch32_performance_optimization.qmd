...
#| echo: true
#| code-fold: true
#| code-summary: "Show multi-stage vector retrieval architecture"
...
coarse_idx = retriever.coarse_search(query, candidates.unsqueeze(0))
...
#| echo: true
#| code-fold: true
#| code-summary: "Show index tuning configuration"
...
#| echo: true
#| code-fold: true
#| code-summary: "Show multi-tier caching architecture"
from dataclasses import dataclass, field
...
#| echo: true
#| code-fold: true
#| code-summary: "Show compression techniques architecture"
from dataclasses import dataclass
...
#| echo: true
#| code-fold: true
#| code-summary: "Show distributed query routing architecture"
from dataclasses import dataclass, field
from typing import Optional, Dict, List
from enum import Enum
import torch
...
class GeoDistributedRouter(nn.Module):
    ...
    def route_query(self, query: torch.Tensor, user_location: torch.Tensor) -> torch.Tensor:
        q_encoded = self.query_encoder(query)
        scores = torch.matmul(q_encoded, self.datacenter_embeddings.T)
        combined = torch.cat([q_encoded.expand(-1, self.datacenter_embeddings.size(0), -1),
                             self.datacenter_embeddings.unsqueeze(0).expand(query.size(0), -1, -1)], dim=-1)
        latencies = self.latency_predictor(combined).squeeze(-1)
