...
#| echo: true
#| code-fold: true
#| code-summary: "Show Transaction Autoencoder for Fraud Detection"
import torch
...
#| echo: true
#| code-fold: true
#| code-summary: "Show Quality Control System"
import torch
import torch.nn as nn
import torch.nn.functional as F
import numpy as np
...
class ProductEncoder(nn.Module):
    """Encode products from sensor measurements."""
    def __init__(self, measurement_dim: int = 100, output_dim: int = 128):
        super().__init__()
    ...
    def forward(self, measurements):
        ...
        product_emb = self.measurement_encoder(measurements)
        return F.normalize(product_emb, p=2, dim=1)


class QualityControlSystem:
    """Manufacturing quality control using embeddings."""
    def __init__(self, embedding_dim: int = 128):
        self.embedding_dim = embedding_dim
        self.encoder = ProductEncoder(measurement_dim=100, output_dim=embedding_dim)
        self.encoder.eval()
        self.cluster_centroid = None
        self.cluster_radius = None

    def build_normal_cluster(self, normal_products):
        """Build cluster from defect-free products."""
        embeddings = []
        for measurements in normal_products:
            measurements_tensor = torch.from_numpy(measurements).unsqueeze(0).float()
            with torch.no_grad():
                emb = self.encoder(measurements_tensor)
        ...
        self.cluster_radius = np.percentile(distances, 95)  # 95th percentile

    def inspect_product(self, measurements):
        """Inspect product for defects."""
        measurements_tensor = torch.from_numpy(measurements).unsqueeze(0).float()
        with torch.no_grad():
            emb = self.encoder(measurements_tensor).numpy()[0]
...
# Usage example
qc_system = QualityControlSystem(embedding_dim=64)

# Build baseline from 100 normal products
normal_products = [np.random.randn(100).astype(np.float32) * 0.1 for _ in range(100)]
qc_system.build_normal_cluster(normal_products)

# Test normal product
test_normal = np.random.randn(100).astype(np.float32) * 0.1
is_def, dist = qc_system.inspect_product(test_normal)
print(f"Normal product - Defective: {is_def}, Distance: {dist:.4f}")

# Test defective product
test_defect = np.random.randn(100).astype(np.float32) * 2.0
is_def, dist = qc_system.inspect_product(test_defect)
print(f"Defective product - Defective: {is_def}, Distance: {dist:.4f}")
```
...
#| echo: true
#| code-fold: true
#| code-summary: "Show Risk Assessment System"
import torch
import torch.nn as nn
import torch.nn.functional as F
import numpy as np
...
class FinancialEntityEncoder(nn.Module):
    """Encode financial entities for risk assessment."""
...
class RiskAssessmentSystem:
    """Financial risk assessment using embeddings."""
    def __init__(self, embedding_dim: int = 128):
        self.embedding_dim = embedding_dim
        self.encoder = FinancialEntityEncoder(embedding_dim=embedding_dim)
        self.encoder.eval()
        self.risk_centroids = {}  # low, medium, high

    def build_risk_clusters(self, entities_by_risk):
        """Build risk clusters from labeled entities."""
        for risk_level, entities in entities_by_risk.items():
            embeddings = []
            for features in entities:
...
# Usage example
risk_system = RiskAssessmentSystem(embedding_dim=64)

# Build risk clusters
entities_by_risk = {
    'low': [np.array([0.2, 0.9] + [0.0] * 48, dtype=np.float32) for _ in range(50)],
    'medium': [np.array([0.4, 0.7] + [0.0] * 48, dtype=np.float32) for _ in range(50)],
    'high': [np.array([0.8, 0.4] + [0.0] * 48, dtype=np.float32) for _ in range(50)]
}
risk_system.build_risk_clusters(entities_by_risk)

# Assess new entity
test_entity = np.array([0.25, 0.95] + [0.0] * 48, dtype=np.float32)
risk, distances = risk_system.assess_risk(test_entity)
