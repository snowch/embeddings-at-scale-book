...
We synthesize different audio signals and extract MFCC features
to create embeddings that capture timbral characteristics.
"""
...
# Synthesize audio signals with different characteristics
sr = 22050  # Sample rate
duration = 1.0  # 1 second
t = np.linspace(0, duration, int(sr * duration))

# Pure tone (simple sine wave at 440 Hz - concert A)
pure_tone = np.sin(2 * np.pi * 440 * t).astype(np.float32)

# Same pitch, different octave (880 Hz)
octave_up = np.sin(2 * np.pi * 880 * t).astype(np.float32)

# Rich harmonic tone (fundamental + harmonics, like a musical instrument)
rich_tone = (np.sin(2 * np.pi * 440 * t) +
             0.5 * np.sin(2 * np.pi * 880 * t) +
             0.25 * np.sin(2 * np.pi * 1320 * t)).astype(np.float32)

# Noise burst (like percussion)
np.random.seed(42)
noise = np.random.randn(len(t)).astype(np.float32) * 0.5

# Generate embeddings
embeddings = {
    'pure_440hz': audio_to_embedding(pure_tone, sr),
    'pure_880hz': audio_to_embedding(octave_up, sr),
    'rich_tone': audio_to_embedding(rich_tone, sr),
    'noise': audio_to_embedding(noise, sr),
}

print(f"Embedding dimension: {len(embeddings['pure_440hz'])} (20 MFCCs × 2 stats)\n")

# Compare similarities - similar timbres should cluster
print("Audio embedding similarities:")
sim_pure = cosine_similarity(
    [embeddings['pure_440hz']], [embeddings['pure_880hz']]
)[0][0]
print(f"  Pure 440Hz ↔ Pure 880Hz:   {sim_pure:.3f}  (same timbre, different pitch)")

sim_rich = cosine_similarity(
    [embeddings['pure_440hz']], [embeddings['rich_tone']]
)[0][0]
print(f"  Pure tone ↔ Rich tone:     {sim_rich:.3f}  (different timbre)")

sim_noise = cosine_similarity(
    [embeddings['pure_440hz']], [embeddings['noise']]
)[0][0]
print(f"  Pure tone ↔ Noise:         {sim_noise:.3f}  (very different)")
```

Pure tones at different pitches remain similar (same timbre), while adding harmonics or noise creates distinct embeddings.
