...
The sentence-transformers models are built on Sentence-BERT [@reimers2019sentencebert], which adapts BERT for efficient sentence embeddings using siamese networks. MiniLM [@wang2020minilm] uses knowledge distillation for a smaller, faster model, while MPNet [@song2020mpnet] combines masked and permuted language modeling for improved representations.
...
classifier = EmbeddingClassifier(MockEncoder(), k=3)
classifier.fit(
...
clusterer = EmbeddingClusterer(MockEncoder(), n_clusters=3)
...
# Example usage
analyzer = SentimentAnalyzer(MockEncoder())
