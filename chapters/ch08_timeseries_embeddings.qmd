...
Time-series with the same pattern type have high similarity because they share statistical properties. The statistical approach above is simple but limited—modern approaches use learned embeddings from neural networks trained on large time-series datasets.

## When to Use Time-Series Embeddings

- **Anomaly detection**—identify unusual patterns in sensor data (see @sec-manufacturing-industry40)
- **Predictive maintenance**—find similar equipment behavior patterns
- **Financial pattern recognition**—detect trading patterns and market regimes (see @sec-financial-services)
- **Health monitoring**—classify ECG, EEG, and other biosignals (see @sec-healthcare-life-sciences)
- **IoT device fingerprinting**—identify devices by their traffic patterns
...
## Advanced: Learned Time-Series Embeddings {.unnumbered}
